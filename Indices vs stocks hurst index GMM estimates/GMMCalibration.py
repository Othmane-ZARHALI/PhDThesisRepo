...
def compute_equal_perc(rng):
    ...
    range_values = [v for v in range(0, rng)]
...
class GMM:
    ...
    def ModelMoments(self,H, lambda2, T, lsigma2, Delta=0.01, LagSignal=[0, 1, 2, 3, 4, 5, 6], flagLambda2=True):
        '''
        --------------------------------------------------------------------
        ...
        flagHighFreq = False => Uses Low-freq formula
        ...
        ee1 = 2 * HH + 1
        ee2 = 2 * HH + 2
        #print("LagSignal,Delta = ",LagSignal,Delta)
    ...
    def ErrorVecSignal(self,logvol_samples_or_covtheoretical, H, lambda2, T, lsigma2=-10, lagSignal=[1, 2, 4, 8, 10], flagErr=True, flagLambda2=True):
        '''
        --------------------------------------------------------------------
        ...
        --------------------------------------------------------------------
        '''
        #print("H,lambda2 = ",H,lambda2)
        if (flagErr):
            # moments_model = self.ModelMoments(H=H, lambda2=lambda2, T=T, lsigma2=lsigma2, lagSig=lagSignal,flagLambda2=flagLambda2)
            moments_model = self.ModelMoments(H=H, lambda2=lambda2, T=T, lsigma2=lsigma2, Delta=0.01,
                                              LagSignal=lagSignal, flagLambda2=flagLambda2)
        else:
            moments_model = np.zeros(len(lagSignal))
        # moments_model = self.ModelMoments(H=H, lambda2=lambda2, T=T, lsigma2=lsigma2, Delta=1,
        #                                   LagSignal=lagSignal, flagLambda2=flagLambda2)
        if type(logvol_samples_or_covtheoretical)==list:
            logvol_samples=logvol_samples_or_covtheoretical
            ...
            for i in range(2, len(lagSignal)):
                ...
                mm = np.vstack((mm, m3))
            return np.array(mm)
        if type(logvol_samples_or_covtheoretical)==types.LambdaType:
            # print("H,lambda2,T,lsigma2 = ",H,lambda2,T,lsigma2)

            theoretical_cov_function = logvol_samples_or_covtheoretical
            mm = []
            theoretical_cov = []
            for i in range(len(lagSignal)):
                # print("theoretical_cov_function(lagSignal[i]),moments_model[i] = ",theoretical_cov_function(lagSignal[i]),moments_model[i])
                m3 = theoretical_cov_function(lagSignal[i]) - moments_model[i]
                theoretical_cov.append(theoretical_cov_function(lagSignal[i]))
                # print("m3,moments_model[i] = ",m3,moments_model[i])
                # mm = np.vstack((mm, m3))
                mm += [[m3]]
                # mm += [m3]
            # print("mm=",mm)
            ...
            # print("moments_model = ", moments_model)
            # print("theoretical_cov = ",theoretical_cov)

            # print("lagSignal = ",lagSignal)
            # print("mm = ", mm)
            # print("******************************")
            return np.array(mm)


    def ErrorVecSignal_m(self,logvol_samples_or_covtheoretical, H, lambda2, lsigma2=-10, lagSignal=[1, 2, 4, 8, 10], qval=2, flagErr=True, flagLambda2=True):
        '''
        --------------------------------------------------------------------
        This function computes the vector of moment errors
        --------------------------------------------------------------------
        ...
        if type(logvol_samples_or_covtheoretical) == list:
            ...
            m1 = (np.abs(xm[:, lagSignal[0]:xm_size + lagSignal[0] - lagMax] - xm[:, :xm_size - lagMax]) ** qval).mean(
                axis=0) - \
                 moms_model_m[0]
            m2 = (np.abs(xm[:, lagSignal[1]:xm_size + lagSignal[1] - lagMax] - xm[:, :xm_size - lagMax]) ** qval).mean(
                axis=0) - \
                 moms_model_m[1]
            mm = np.vstack((m1, m2))
            for i in range(2, len(lagSignal)):
                m3 = (np.abs(
                    xm[:, lagSignal[i]:xm_size + lagSignal[i] - lagMax] - xm[:, :xm_size - lagMax]) ** qval).mean(
        ...
        if type(logvol_samples_or_covtheoretical)==types.LambdaType:
            ...
            mm = np.array([])
            for i in range(2, len(lagSignal)):
                m3 = theoretical_cov_function(lagSignal[i]) - moms_model_m[i]
                mm = np.vstack((mm, m3))
            return np.array(mm)



    def ErrorVecSignal_M(self,logvol_samples_or_covtheoretical, H, lambda2, K1, T, lagSignal=[1, 2, 4, 8, 10], flagErr=True, flagLambda2=True):
        '''
        --------------------------------------------------------------------
        This function computes the vector of moment errors
        --------------------------------------------------------------------
        ...
        if type(logvol_samples_or_covtheoretical) == list:
            ...
            m1 = (xm[:, 0:xm_size - lagMax1] * xm[:, lagSignal[0]:lagSignal[0] + xm_size - lagMax1]).mean(
                axis=0) / m0.mean(
                axis=0) - moms_model[0]

            m2 = (xm[:, 0:xm_size - lagMax1] * xm[:, lagSignal[1]:lagSignal[1] + xm_size - lagMax1]).mean(
                axis=0) / m0.mean(
                axis=0) - moms_model[1]
            mm = np.vstack((m1, m2))
            for i in range(2, len(lagSignal)):
                m3 = (xm[:, 0:xm_size - lagMax1] * xm[:, lagSignal[i]:lagSignal[i] + xm_size - lagMax1]).mean(
                    axis=0) / m0.mean(
                    axis=0) - moms_model[i]
                mm = np.vstack((mm, m3))
            return np.array(mm)
        if type(logvol_samples_or_covtheoretical)==types.LambdaType:
            ...
            mm = np.array([])
            for i in range(2, len(lagSignal)):
                m3 = theoretical_cov_function(lagSignal[i]) - moms_model[i]
                mm = np.vstack((mm, m3))
            return np.array(mm)
    ...
    def criterion(self,params, *args):
        ...
        '''
        H, lambda2, T = params
        logvol_samples_or_covtheoretical, lagSig, lsigma2, W, flagLambda2 = args
        err = self.ErrorVecSignal(logvol_samples_or_covtheoretical, H, lambda2, T, lsigma2, lagSig, flagLambda2=flagLambda2)

        err = err.mean(axis=1)
        crit_val = np.dot(np.dot(err.T, W), err)
        crit_val = (np.abs(crit_val))
        print("crit_val = ", crit_val)
        #return -1 * np.log(1.0 / crit_val - 1) #np.exp(crit_val)  #np.log(crit_val)

        # return crit_val+0*np.dot(err.T, err)+0.1*np.linalg.norm(err, ord=1)  # np.exp(crit_val)  #np.log(crit_val)  # 0.01* gives the same H for same H components
        return crit_val + 0 * np.dot(err.T, err) + np.sum(np.array(compute_equal_perc(len(err)))*np.absolute(err)) # np.exp(crit_val)  #np.log(crit_val)  # 0.01* gives the same H for same H components
    ...
    def criterion_M(self,params, *args):
        ...
        '''
        H, lambda2, K1 = params
        logvol_samples_or_covtheoretical, T, lagSig, W, flagLambda2 = args

        err = self.ErrorVecSignal_M(logvol_samples_or_covtheoretical, H, lambda2, K1, T, lagSig, flagLambda2=flagLambda2)
        err = err.mean(axis=1)
    ...
    def criterion_m(self,params, *args):
        ...
        logvol_samples_or_covtheoretical, lagSig, qvals, W, flagLambda2 = args

        err = self.ErrorVecSignal_m(logvol_samples_or_covtheoretical, H, lambda2, lsigma2, lagSig, qvals, flagLambda2=flagLambda2)
        err = err.mean(axis=1)
    ...
    def ComputeParamsGMM(self,datastream_sample_or_covtheoretical, niter=5, LagSignal=np.array([1, 2, 4, 5, 8, 11, 16, 22, 32, 45, 64, 90, 128]),
        ...
        ### For J-test
        J = 0
        if type(datastream_sample_or_covtheoretical)==list:
            datastream_sample = datastream_sample_or_covtheoretical
            ...
            if (GMM_Method == 0):
                ...
                res = opt.minimize(self.criterion_M, params_init, args=(gmm_args), method=method, tol=tol,
                                   bounds=bounds)
            ...
            for i in range(niter):
                ...
                if (GMM_Method == 0):
                    err1 = self.ErrorVecSignal_M(datastream_sample, H_gmm1, lambda2_gmm1, T_gmm1, np.log(LL), LagSignal,
                                                 flagLambda2)

                if (GMM_Method == 1):
                    err1 = self.ErrorVecSignal(datastream_sample, H_gmm1, lambda2_gmm1, T_gmm1, lsigma2_gmm1, LagSignal,
                                               flagLambda2)

                if (GMM_Method == 2):
                    err1 = self.ErrorVecSignal_m(datastream_sample, H_gmm1, lambda2_gmm1, lsigma2_gmm1, LagSignal,
                                                 qvals, flagLambda2)
                ...
                if (GMM_Method == 0):
                    ...
                    J = self.criterion_M(np.array([H_gmm1, lambda2_gmm1, T_gmm1]), datastream_sample, np.log(LL),
                                         LagSignal, W_hat, flagLambda2)
                ...
                if (GMM_Method == 1):
                    ...
                    # J = criterion(np.array([H_gmm1,lambda2_gmm1,lsigma2_gmm1]),xvals,T_gmm1,lagSig,W_hat,flagLambda2)
                    J = self.criterion(np.array([H_gmm1, lambda2_gmm1, T_gmm1]), datastream_sample, LagSignal,
                                       lsigma2_gmm1, W_hat, flagLambda2)
                    J *= len(datastream_sample[0])
                ...
                if (GMM_Method == 2):
                    ...
                    # res = opt.minimize(criterion_m,params_init,args=(gmm_args),method=method,tol=tol)
                    H_gmm1, lambda2_gmm1, lsigma2_gmm1 = res.x
                    J = self.criterion_m(np.array([H_gmm1, lambda2_gmm1, lsigma2_gmm1]), datastream_sample, LagSignal,
                                         qvals, W_hat,
                                         flagLambda2)
                    J *= len(datastream_sample[0])
            ...
            if (flagPlot):
                ...
                if (GMM_Method == 1):
                    ...
                    if (ss[0] == 0):
                        ss1 = ss[1:]
                    err1 = self.ErrorVecSignal(datastream_sample, H_gmm1, lambda2_gmm1, T_gmm1, lsigma2_gmm1, ss,
                                               flagErr=False, flagLambda2=flagLambda2)
                    cc1 = err1.mean(axis=1)
                    ### Model Theoretical Covariance
                    mm = self.ModelMoments(H_gmm1, lambda2_gmm1, T_gmm1, lsigma2_gmm1, LagSignal=ss,
                    ...
                    mm = self.ModelMoments(H_gmm1, lambda2_gmm1, T_gmm1, lsigma2_gmm1, LagSignal=ss1,
                                           flagLambda2=flagLambda2)
                    err2 = self.ErrorVecSignal(datastream_sample, H_gmm1, lambda2_gmm1, T_gmm1, lsigma2_gmm1, ss1,
                                               flagErr=False,
                                               flagLambda2=flagLambda2)
                ...
                if (GMM_Method == 0):
                    ...
                    err1 = self.ErrorVecSignal_M(datastream_sample, H_gmm1, lambda2_gmm1, T_gmm1, np.log(LL), ss,
                                                 flagErr=False, flagLambda2=flagLambda2)
                    cc1 = err1.mean(axis=1)
                    ### Model Theoretical Covariance
                    mm = self.ModelMoments_M(H_gmm1, lambda2_gmm1, T_gmm1, np.log(LL), LagSignal=ss,
                                             flagLambda2=flagLambda2)
                    ...
                    mm = self.ModelMoments_M(H_gmm1, lambda2_gmm1, T_gmm1, np.log(LL), LagSignal=ss1,
                                             flagLambda2=flagLambda2)
                    err2 = self.ErrorVecSignal_M(datastream_sample, H_gmm1, lambda2_gmm1, T_gmm1, np.log(LL), ss1,
                                                 flagErr=False, flagLambda2=flagLambda2)
                    cc2 = err2.mean(axis=1)
                    plt.figure()
                ...
                if (GMM_Method == 2):
                    err1 = self.ErrorVecSignal_m(datastream_sample, H_gmm1, lambda2_gmm1, lsigma2_gmm1, LagSignal,
                                                 qvals, flagErr=False, flagLambda2=flagLambda2).mean(axis=1)
                    dd = np.zeros((1, len(LagSignal)))
                    xx = np.zeros((1, len(LagSignal)))
                    for i in range(1):
                        dd[i, :] = err1[i * len(LagSignal):(i + 1) * len(LagSignal)]
                        xx[i, :] = self.ModelMoments_m(H_gmm1, lambda2_gmm1, lsigma2_gmm1, LagSignal=LagSignal,
                                                       qval=qvals, flagLambda2=flagLambda2)
                ...
                ### Sargan-Hansen Test
            if (flagLambda2):
                print('H = ', HH_gmm1, 'lambda2 =', (l2_gmm1), 'T = ', np.exp(T_gmm1), 'lsigma2 =', ls2_gmm1,
                      'p-Value= ',
        ...
        if type(datastream_sample_or_covtheoretical) == types.LambdaType:
            covtheoretical = datastream_sample_or_covtheoretical
            ### Case we have a single array
            ...
            if (GMM_Method == 1):
                # params_init = np.array([H,lambda2,T0,lsigma2])
                params_init = np.array([zz, zl, T_gmm1])
                ...
                W_hat = np.eye(len(LagSignal))
                gmm_args = (covtheoretical, LagSignal, lsigma2_gmm1, W_hat, flagLambda2)
                res = opt.minimize(self.criterion, params_init, args=(gmm_args), method=method, tol=tol, bounds=bounds) #,options={'maxiter':20}
                H_gmm1, lambda2_gmm1, T_gmm1 = res.x
                print('*******************************')
                print("res = ", res)
                print('*******************************')
            ...
            if (GMM_Method == 0):
                ...
                # gmm_args = (covtheoretical, np.log(LL), LagSignal, W_hat, flagLambda2)
                gmm_args = (covtheoretical, LagSignal, lsigma2_gmm1, W_hat, flagLambda2)
                res = opt.minimize(self.criterion_M, params_init, args=(gmm_args), method=method, tol=tol,
                                   bounds=bounds)
                # res = opt.minimize(criterion_M,params_init,args=(gmm_args),method=method,tol=tol)
            ...
            if (GMM_Method == 2):
                ...
                W_hat = np.eye(len(LagSignal))
                gmm_args = (covtheoretical, LagSignal, qvals, W_hat, flagLambda2)
                res = opt.minimize(self.criterion_m, params_init, args=(gmm_args), method=method, tol=tol,
                                   bounds=bounds)
                # res = opt.dual_annealing(criterion_m,x0=params_init,args=(gmm_args),tol=tol,method=method)
                H_gmm1, lambda2_gmm1, lsigma2_gmm1 = res.x
            ...
            for i in range(niter):
                ...
                if (GMM_Method == 0):
                    err1 = self.ErrorVecSignal_M(covtheoretical, H_gmm1, lambda2_gmm1, T_gmm1, lsigma2_gmm1, LagSignal,
                                                 flagLambda2)

                if (GMM_Method == 1):
                    # print("H_gmm1, lambda2_gmm1, T_gmm1, lsigma2_gmm1 = ",H_gmm1, lambda2_gmm1, T_gmm1, lsigma2_gmm1)
                    err1 = self.ErrorVecSignal(covtheoretical, H_gmm1, lambda2_gmm1, T_gmm1, lsigma2_gmm1, LagSignal,
                                               flagLambda2)
                    print("err1 = ", err1)
                    # print("err1.mean(axis=1) = ",err1.mean(axis=1))
                    # print("err1.mean(axis=0) = ", err1.mean(axis=0))

                if (GMM_Method == 2):
                    err1 = self.ErrorVecSignal_m(covtheoretical, H_gmm1, lambda2_gmm1, lsigma2_gmm1, LagSignal,
                                                 qvals, flagLambda2)
                ...
                #### Compute Hac Estimation (Newey-West Kernel)
                # err2 = np.subtract(err1.T, err1.mean(axis=1))
                err2 = np.subtract(err1.T, err1.mean(axis=0))
                err2 = err2.T
                # print("err2 = ", err2)
                WW = NeweyWest_CovarianceMatrixEstimation(err2)
                # print("WW = ", WW)
                ...
                if (GMM_Method == 0):
                    ...
                    # J = criterion(np.array([H_gmm1,lambda2_gmm1,lsigma2_gmm1]),xvals,T_gmm1,lagSig,W_hat,flagLambda2)
                    J = self.criterion_M(np.array([H_gmm1, lambda2_gmm1, T_gmm1]), covtheoretical, lsigma2_gmm1,
                                         LagSignal, W_hat, flagLambda2)
                    # J *= len(datastream_sample[0])
                ...
                if (GMM_Method == 1):
                    ...
                    params = np.array([H_gmm1, lambda2_gmm1, T_gmm1])
                    # params = np.array([H_gmm1,lambda2_gmm1,lsigma2_gmm1])
                    res = opt.minimize(self.criterion, params, args=(gmm_args), method=method, tol=tol, bounds=bounds) #,options={'maxiter':20}
                    # res = opt.minimize(criterion,params_init,args=(gmm_args),method=method,tol=tol)
                    ...
                    # J = criterion(np.array([H_gmm1,lambda2_gmm1,lsigma2_gmm1]),xvals,T_gmm1,lagSig,W_hat,flagLambda2)
                    J = self.criterion(np.array([H_gmm1, lambda2_gmm1, T_gmm1]), covtheoretical, LagSignal,
                                       lsigma2_gmm1, W_hat, flagLambda2)
                    # J *= len(datastream_sample[0])
                ...
                if (GMM_Method == 2):
                    ...
                    # res = opt.minimize(criterion_m,params_init,args=(gmm_args),method=method,tol=tol)
                    H_gmm1, lambda2_gmm1, lsigma2_gmm1 = res.x
                    J = self.criterion_m(np.array([H_gmm1, lambda2_gmm1, lsigma2_gmm1]), covtheoretical, LagSignal,
                                         qvals, W_hat,
                                         flagLambda2)
                    # J *= len(datastream_sample[0])
            ...
            if (flagPlot):
                ...
                if (GMM_Method == 1):
                    ...
                    if (ss[0] == 0):
                        ss1 = ss[1:]
                    err1 = self.ErrorVecSignal(covtheoretical, H_gmm1, lambda2_gmm1, T_gmm1, lsigma2_gmm1, ss,
                                               flagErr=False, flagLambda2=flagLambda2)
                    ...
                    mm = self.ModelMoments(H_gmm1, lambda2_gmm1, T_gmm1, lsigma2_gmm1, LagSignal=ss,
                                           flagLambda2=flagLambda2)
                    print("H_gmm1, lambda2_gmm1 = ",H_gmm1, lambda2_gmm1)
                    print("err1 final = ", err1)
                    print("mm final = ", mm)
                    plt.plot(ss, cc1, 'o-', color='red', label='Empirical mean error GMM')
                    plt.plot(ss, mm, label='Model moments GMM')
                    plt.legend()
                    ...
                    mm = self.ModelMoments(H_gmm1, lambda2_gmm1, T_gmm1, lsigma2_gmm1, LagSignal=ss1,
                                           flagLambda2=flagLambda2)
                    err2 = self.ErrorVecSignal(covtheoretical, H_gmm1, lambda2_gmm1, T_gmm1, lsigma2_gmm1, ss1,
                                               flagErr=False,
                                               flagLambda2=flagLambda2)
                    cc2 = err2.mean(axis=1)
                    plt.figure()
                ...
                if (GMM_Method == 0):
                    ...
                    err1 = self.ErrorVecSignal_M(covtheoretical, H_gmm1, lambda2_gmm1, T_gmm1,lsigma2_gmm1, ss,
                                                 flagErr=False, flagLambda2=flagLambda2)
                    cc1 = err1.mean(axis=1)
                    ### Model Theoretical Covariance
                    mm = self.ModelMoments_M(H_gmm1, lambda2_gmm1, T_gmm1, lsigma2_gmm1, LagSignal=ss,
                                             flagLambda2=flagLambda2)
                    plt.plot(ss, cc1, 'o-', label='Empirical mean error GMM')
                    plt.plot(ss, mm, label='Model moments GMM')
                    plt.legend()
                    ...
                    mm = self.ModelMoments_M(H_gmm1, lambda2_gmm1, T_gmm1, np.log(LL), LagSignal=ss1,
                                             flagLambda2=flagLambda2)
                    err2 = self.ErrorVecSignal_M(covtheoretical, H_gmm1, lambda2_gmm1, T_gmm1, np.log(LL), ss1,
                                                 flagErr=False, flagLambda2=flagLambda2)
                    cc2 = err2.mean(axis=1)
                    plt.figure()
                    plt.plot(np.log(ss1), cc2, 'o-', label='Empirical mean error GMM - log scale')
