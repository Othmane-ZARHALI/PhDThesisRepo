...
def g_i_j_calibration(tau,Delta,H_list,T,lambda_i,lambda_j,g_i_j_list,N_maxtimestep,N_maxMC,Flag_variance_reduction = False,regime="general intermittencies"):
    ...
    if regime == "general intermittencies":
        if Flag_variance_reduction == False:
            ...
            return estimate,(variance/N_maxMC)**(1/2)
        if Flag_variance_reduction == True:
            ...
            variance = np.var((1 / (lambda_i * lambda_j)) * ((MC_copies_i * MC_copies_j+MC_copies_i_minus * MC_copies_j_minus)/2) / (
                        gamma_2_ij * Delta ** 2 - gamma_1_ij * Delta ** 2 * tau ** (H_list[0] + H_list[1]) * (
                            2 + (H_list[0] + H_list[1])) * (1 + (H_list[0] + H_list[1]))))
            return estimate, (variance / N_maxMC)**(1/2)
    if regime == "Small intermittencies":
        if Flag_variance_reduction == False:
            for k in range(N_maxMC):
                log_MRM_i = TwoDimentionalPathsSimulation(0, Delta, H_list, T, lambda_i, lambda_j, g_i_j_list, N_maxtimestep)[0]
                log_MRM_j = TwoDimentionalPathsSimulation(Delta,tau+Delta, H_list, T, lambda_i, lambda_j, g_i_j_list, N_maxtimestep)[1]
                MC_copies_i.append(np.exp(log_MRM_i).sum() * (Delta) / N_maxtimestep)
                MC_copies_j.append(np.exp(log_MRM_j).sum() * (Delta) / N_maxtimestep)
            MC_copies_i, MC_copies_j =np.log(np.array(MC_copies_i))/Delta,np.log(np.array(MC_copies_j))/Delta
            MC_copies_i, MC_copies_j = np.array(MC_copies_i), np.array(MC_copies_j)
            estimate = (Delta**2 / N_maxMC) * np.sum(MC_copies_i * MC_copies_j) / (lambda_i*lambda_j*(gamma_2_ij * Delta ** 2 - gamma_1_ij * Delta ** 2 * tau ** (H_list[0] + H_list[1]) * (2 + (H_list[0] + H_list[1])) * (1 + (H_list[0] + H_list[1]))))
            variance = np.var((MC_copies_i * MC_copies_j / (lambda_i*lambda_j*(gamma_2_ij * Delta ** 2 - gamma_1_ij * Delta ** 2 * tau ** (H_list[0] + H_list[1]) * (2 + (H_list[0] + H_list[1])) * (1 + (H_list[0] + H_list[1]))))))
            return estimate, (variance / N_maxMC) ** (1 / 2)
...
#print(g_i_j_calibration(100,5,[0.01,0.02],150,0.001,0.001,0.01,100,500,True))
print(g_i_j_calibration(100,5,[0.01,0.02],150,0.001,0.001,0.01,100,100,False,"Small intermittencies"))
...
def g_i_j_calibrationStatistics(tau,Delta,H_list,T,lambda_i,lambda_j,g_i_j_list,N_maxtimestep,N_maxMC,N_MCestim,outputtype='Mean'):
    ...
    for k in range(N_MCestim):
        S.append(g_i_j_calibration(tau, Delta, H_list, T, lambda_i, lambda_j, g_i_j_list, N_maxtimestep, N_maxMC)[0])
    if outputtype=='Mean':
    ...
    if outputtype=='Histogram':
        ...
        plt.savefig('g_ijHistogram.png')
        #plt.show()
        return "plotted"


#print(g_i_j_calibrationStatistics(100,5,[0.01,0.02],150,0.001,0.001,0.01,100,500,300,"Histogram"))
